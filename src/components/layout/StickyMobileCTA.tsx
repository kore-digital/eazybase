 ...
 * Call / WhatsApp / Get A Quote. Hides itself while the on-page quote form
 * (any element marked `data-quote-form` or `#quote-form`) is in view.
 * Give page content bottom padding (or rely on the footer) so nothing is
...
export function StickyMobileCTA() {
  ...
  useEffect(() => {
    const form =
      document.querySelector('[data-quote-form]') ?? document.getElementById('quote-form')
    if (!form) return
    ...
    >
      <div className="grid grid-cols-3 border-t border-ink-100 bg-white shadow-[0_-4px_16px_rgba(30,31,29,0.12)]">
        ...
        <a
          ...
          className="flex items-center justify-center gap-1.5 bg-brand-500 py-3.5 font-display text-xs font-semibold tracking-wide text-white uppercase"
        >
